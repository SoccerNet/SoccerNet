...
def inferListGame(SoccerNet_path):
    if zipfile.is_zipfile(SoccerNet_path):
        ...
        with zipfile.ZipFile(zip_folder_path, 'r') as zip_ref:
            ...
            label_files = os.path.basename(list_json[0]) if len(list_json) > 0 else None

            
    else:
        list_games = []
        print("SoccerNet_path", SoccerNet_path)
        label_files = None
    ...
    for game in list_games:
        print(" --- ", game, label_files)
    return list_games, label_files
...
def evaluate(
        ...
        framerate=2, 
        metric="loose", 
        label_files="Labels-v2.json", # DEPRECATED, set to None and use inferGameList instead. Kept for backward compatibility
    ...
    if dataset is None or label_files is None:
        list_games, label_files = inferListGame(SoccerNet_path=SoccerNet_path)
    else:
